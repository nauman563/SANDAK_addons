...
class ZK(object):
    ...
    def connect(self):
        ...
        _logger.info("/34444444444444444444444444444")
        self.__create_socket()  # tcp based
        _logger.info("/5555555555555555555555555555555")
        self.__session_id = 0
        self.__reply_id = const.USHRT_MAX - 1
        _logger.info("/666666666666666666666666666 %s %s", self.__send_command, const.CMD_CONNECT)
        cmd_response = self.__send_command(const.CMD_CONNECT)
        _logger.info("/777777777777777777777777777777777777")
        self.__session_id = self.__header[2]
        _logger.info("/3 CMD Response %s", cmd_response)
